        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-6">
              {[
                { user: 'Mike Tech', action: 'resolved ticket', target: 'T-982', time: '10m ago', icon: CheckCircle2, color: 'text-emerald-500' },
                { user: 'John Doe', action: 'reported issue', target: 'M-105', time: '45m ago', icon: AlertTriangle, color: 'text-red-500' },
                { user: 'Sarah Smith', action: 'updated status', target: 'T-1001', time: '1h ago', icon: Clock, color: 'text-amber-500' },
                { user: 'System', action: 'scheduled maintenance', target: 'M-104', time: '3h ago', icon: Settings, color: 'text-slate-500' },
              ].map((item, i) => (
                <div key={i} className="flex gap-4">
                  <div className={cn('mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-slate-100 dark:bg-slate-900', item.color)}>
                    <item.icon className="h-4 w-4" />
                  </div>
                  <div className="space-y-1">
                    <p className="text-sm font-medium">
                      <span className="font-bold">{item.user}</span> {item.action} <span className="font-bold text-indigo-600 dark:text-indigo-400">{item.target}</span>
                    </p>
            ...
            <CardFooter>
              <Button variant="ghost" className="w-full text-xs text-indigo-600 dark:text-indigo-400">View All Activity</Button>
            </CardFooter>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-indigo-600 to-violet-700 text-white">
            <CardHeader>
              ...
              <CardDescription className="text-indigo-100">Contact the supervisor on duty</CardDescription>
            </CardHeader>
            ...
            <CardFooter>
              <Button className="w-full bg-white text-indigo-600 hover:bg-indigo-50">Call Supervisor</Button>
            </CardFooter>
          </Card>
        </div>
      </div>
